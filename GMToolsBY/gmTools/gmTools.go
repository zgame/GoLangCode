		...
		Children: []Widget{
			...
			VSplitter{
				...
				Children: []Widget{
					...
					//
					//---------------------------------------------------------------------------
					TextEdit{AssignTo: &searchTE,
						Text: "", Enabled: true},
					PushButton{
						Text: "搜索符合条件服务器并选中",
						OnClicked: func() {
							fmt.Println("输入查询条件为:",searchTE.Text())
							for _,v:=range mw.model.items{
								v.checked = false			// 先取消所有的选择
							}

							for _,v:= range mw.model.items{
								if strings.Contains(v.ServerName, searchTE.Text()){
									v.checked = true
								}
							}
							mw.model.PublishRowsReset()
