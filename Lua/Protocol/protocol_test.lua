---
--- Generated by EmmyLua(https://github.com/EmmyLua)
--- Created by Administrator.
--- DateTime: 2018/10/12 15:56
---

local pb = require("pb")
local user_pb = require('user_pb')

local person= user_pb.Friend()
person.zswnameb = -855.9687
person.address = false
person.ad = 1
--person.ad1 = -562949953421312
person.ad1:append(0)
--sssd:append(0)
--print()
--person.ad1 = {0,0}

person.ad2 = 1125899906842624
person.ad3 = 4294967295
--person.ad4 = -2147483647
person.ad4:append(0)
person.ad4:append(10)
person.ad4:append(11)


person.ad5 = "sdf只是问问二翁绕弯儿翁"
person.ad6 = -2147483647
person.ad7 = 562949953421312
person.ad8 = 2147483647
person.ad9 = 562949953421312
person.ad10 = string.char(112,113,114,115)


local zzz = person.zz:add()
zzz.name = "sdfsd这是"
zzz.id = 21343
local zzz2 = person.zz:add()
zzz2.name = "2这是"
zzz2.id = 2

local zzz3 = person.zz1
zzz3.id = 9999



local data = person:SerializeToString()
print("***************************************----SerializeToString-----data----")
pb.ZswLuaShowBytesToString(data)



local msg = user_pb.Friend()
msg:ParseFromString(data)
print("msg**********************************************************************",msg.zswnameb)
print("msg**********************************************************************",msg.address)
print("msg**********************************************************************",msg.ad)
print("msg*********************************************ad1*************************",msg.ad1)
print("msg**********************************************************************",msg.ad2)
print("msg**********************************************************************",msg.ad3)
print("msg**********************************************ad4************************",msg.ad4)
print("msg**********************************************************************",msg.ad5)
print("msg**********************************************************************",msg.ad6)
print("msg**********************************************************************",msg.ad7)
print("msg**********************************************************************",msg.ad8)
print("msg**********************************************************************",msg.ad9)
print("msg**********************************************************************",msg.ad10)
print("msg**********************************************************************",msg.zz[1].name)
print("msg**********************************************************************",msg.zz[1].id)
print("msg**********************************************************************",msg.zz[2].name)
print("msg**********************************************************************",msg.zz[2].id)
print("msg**********************************************************************",msg.zz1.id)

print("000000000000000000000000000000*****************************************************************000000000000000")
--
local CMD_Game_pb = require('CMD_Game_pb')
--
local mm = CMD_Game_pb.tagSceneFish()
mm.uid = 123

local data = mm:SerializeToString()
local msg = CMD_Game_pb.tagSceneFish()
msg:ParseFromString(data)
print("msg**********************************************************************",msg.uid)

--
--
--local sendCmd = CMD_Game_pb.CCMD_S_DISTRIBUTE_FISH()
--local cmd = sendCmd.fishs:add()
--cmd.uid = 1
--cmd.kind_id = 1
--local cmd2 = sendCmd.fishs:add()
--cmd2.uid = 222
--cmd2.kind_id = 234
--
--local data = sendCmd:SerializeToString()
--local msg = CMD_Game_pb.CCMD_S_DISTRIBUTE_FISH()
--msg:ParseFromString(data)
--print("msg**********************************************************************",msg.fishs[2].uid)

--
--
function zsws(sendCmd)
    --sendCmd.num22 = 110
    return sendCmd
end
print("000000000000000000000000--------------------------------开始测试结构体为空---------------------------------000000000000000000000")

local sendCmd = CMD_Game_pb.CCMD_S_DISTRIBUTE_FISH()
sendCmd = zsws(sendCmd)

local data = sendCmd:SerializeToString()
local msg = CMD_Game_pb.CCMD_S_DISTRIBUTE_FISH()
msg:ParseFromString(data)
print("000000000000000000000000-----------------------------------------------------------------000000000000000000000")