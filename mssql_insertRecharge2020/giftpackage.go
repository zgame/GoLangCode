...
// 礼包处理
func GetGiftPackageRechargeSql(rechargeInfo RechargeList, dbNow *sql.DB, dataTimeStr string, dbName string, day1 string, TestDB *sql.DB) {
	// 变量初始化
	getScore := 0   // 金币
	getDiamond := 0 // 钻石
	getCoin := 0    // 灵力
	...
	case 10020: // 金币  灵力
		...
		ItemArray = AddItemArray(109, 3, ItemArray)
	case 10021: // 金币  灵力
		getScore = 8400000
		getCoin = 300000
		title = "充值每日198元礼包3赠送"
		...
		ItemArray = AddItemArray(109, 4, ItemArray)
	case 10022: // 金币 钻石  灵力
		getScore = 24000000
		getDiamond = 1000
		getCoin = 500000
	...
	for _, item := range ItemArray {
		...
		if item.ItemId >=108 && item.ItemId <=111 {
			...
			lastAllScore:= GetScoreRechargeSql(rechargeInfo, BombScore, dbNow, dataTimeStr, dbName, day1, 1, 1, "游戏写分", 10,TestDB)
