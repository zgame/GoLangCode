...
func DealUserList(idStart int) {
	...
	for _, userInfo := range dataBaseArray{
		...
		if day1N < 20190409 {
			...
			forwardScore, recordTimeScore := GetForwardScore(logDBName1, dayStart,logDB1,  userInfo) // 获取玩家的最终金币数量
			forwardDiamond, recordTimeDiamond := GetForwardDiamond(logDBName1, dayStart,logDB1,  userInfo) // 获取玩家的最终数量
			forwardCoin, recordTimeCoin := GetForwardCoin(logDBName1, dayStart,logDB1,  userInfo) // 获取玩家的最终数量


			// 历史遗留数据量
			dayStart = "20190408"
			lastAllScore,_ := GetHistoryScore(DataBaseBYDBName, dayStart,DataBaseBYDB,  userInfo,nil) // 获取玩家的历史金币数量
			lastAllDiamond,_ := GetHistoryDiamond(DataBaseBYDBName, dayStart,DataBaseBYDB,  userInfo,nil) // 获取玩家的历史数量
			lastAllCoin,_ := GetHistoryCoin(DataBaseBYDBName, dayStart,DataBaseBYDB,  userInfo,nil) // 获取玩家的历史数量
			...
			for _,itemId := range ItemArray{
				...
				forwardItem, recordTimeItem := GetForwardItem(logDBName1, dayStart,logDB1,  userInfo, itemId) // 获取玩家的最终数量
				// 历史遗留数据量
				dayStart = "20190408"
				lastAllItem,_ := GetHistoryItem(DataBaseBYDBName, dayStart,DataBaseBYDB,  userInfo, itemId, nil) // 获取玩家的历史数量
		...
		if day2N > 20190421{
			...
			forwardScore, _ = GetForwardScore(logDBName1, dayStart,logDB1,  userInfo) // 获取玩家的最终金币数量
			forwardDiamond, _ = GetForwardDiamond(logDBName1, dayStart,logDB1,  userInfo) // 获取玩家的最终数量
			forwardCoin, _ = GetForwardCoin(logDBName1, dayStart,logDB1,  userInfo) // 获取玩家的最终数量
			for _,itemId := range ItemArray{
				forwardItem, _ := GetForwardItem(logDBName1, dayStart,logDB1,  userInfo, itemId) // 获取玩家的最终数量
		...
		// 对尾部数据进行缝合
		lastAllScore,recordTimeScore := GetHistoryScore(DataBaseBYDBName, dayStart ,DataBaseBYDB,  userInfo,logDB1)     // 获取玩家的历史金币数量
		lastAllDiamond,recordTimeDiamond := GetHistoryDiamond(DataBaseBYDBName, dayStart,DataBaseBYDB,  userInfo,logDB1) // 获取玩家的历史数量
		lastAllCoin,recordTimeCoin := GetHistoryCoin(DataBaseBYDBName, dayStart,DataBaseBYDB,  userInfo,logDB1) // 获取玩家的历史数量
		...
		for index,itemId := range ItemArray{
			lastAllItem,recordTimeItem := GetHistoryItem(DataBaseBYDBName, dayStart,DataBaseBYDB,  userInfo, itemId, logDB1) // 获取玩家的历史数量
