...
type Games struct {
	...
	// 玩家
	AllUserList map[int] Common.PlayerInterface // 玩家的总列表 ， key： user id  ，  value：  user的指针

}

func (games *Games)NewGame(name string, gameid int, open bool) *Games {
	return &Games{GameName:name, GameID:gameid, GameSwitch:open, AllTableList:make(map[int]Common.TableInterface), TableUUID:0, AllUserList:make(map[int]Common.PlayerInterface)}
}
...
func  (games *Games)GetUserByUID(uid int) Common.PlayerInterface {
	return games.AllUserList[uid]
}


// 有玩家登陆游戏，想进入对应分数的房间
func (games *Games)PlayerLoginGame(player *UserSave.UserSave, gameScore int) int{
	// 根据游戏类型创建游戏中玩家的句柄
	uid := int(player.UserId)
	userHandle := games.AllTableList[0].GetUserInterfaceHandle() // 获取玩家类型句柄
	user := userHandle.NewPlayer(uid)                            // 创建游戏中玩家数据
...
// 玩家登出
func (games *Games)PlayerLogOutGame( user Common.PlayerInterface)  {
	//user := games.GetUserByUID(uid)
