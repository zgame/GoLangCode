...
const flagArrayCap = 100 // 索引大于100时key用string
...
func (tb *LTable) Len() int {
	if tb.array == nil {
		if tb.dict != nil {
			//fmt.Println("tablego 11Len=", len(tb.dict))
			return len(tb.dict)
		}
		return 0
	}
	...
	for i := len(tb.array) - 1; i >= 0; i-- {
		...
		if prev == LNil && v != LNil {
			...
			if tb.dict != nil {
				j := len(tb.dict)
				//fmt.Println("tablego 222Len=", i + 1 + j)
				return i + 1 + j
			}
			//fmt.Println("tablego 333Len=", i + 1)
			return i + 1
		}
		prev = v
	}
...
func (tb *LTable) RawSet(key LValue, value LValue) {
	...
	case LNumber:
		if isArrayKey(v) {
			...
			//	tb.array = append(tb.array, value)
			case index >= alen:
				if index <= flagArrayCap {
					for i := 0; i < (index - alen); i++ {
						tb.array = append(tb.array, LNil)
					}
					tb.array = append(tb.array, value)
					//fmt.Println("key=",key.String()," set arry val=",value.String())
				} else {
					//fmt.Println("key=",key.String()," set dict val=",value.String())
	...
	case LString:
		...
		//fmt.Println("key=",key.String()," set strdict val=",value.String())
		return
	}
...
func (tb *LTable) RawSetString(key string, value LValue) {
	...
	if value == LNil {
		...
		delete(tb.strdict, key)     									 // remove strdict

		lkey := LString(key)
		if oldIndex,ok := tb.k2i[lkey]; ok {
...
func (tb *LTable) RawGet(key LValue) LValue {
	//fmt.Println("RawGet key=",key.String())
	switch v := key.(type) {
	case LNumber:
		if isArrayKey(v) {
			if tb.array == nil {
			...
			if index < len(tb.array) {
				//fmt.Println("key=",key.String()," return arry val=",tb.array[index])
				return tb.array[index]
			} else {
				//fmt.Println("RawGet key=",key.String()," return dict val=",tb.RawGetH(v))
				return tb.RawGetH(v)
			}
		}
	case LString:
		...
		if ret, ok := tb.strdict[string(v)]; ok {
			//fmt.Println("RawGet key=",key.String()," return strdict val=", ret)
			return ret
		}
		return LNil
	}
	...
	if v, ok := tb.dict[key]; ok {
		//fmt.Println("RawGet key=",key.String()," return dict val=",v.String())
		return v
	}
	return LNil
}
...
func (tb *LTable) RawGetH(key LValue) LValue {
	//fmt.Println("RawGetH key=",key.String())
...
func (tb *LTable) RawGetString(key string) LValue {
	//fmt.Println("RawGetString key=",key)
	if tb.strdict == nil {
		return LNil
	}
...
func (tb *LTable) Next(key LValue) (LValue, LValue) {
	//fmt.Println("Next key = ", key.String())
