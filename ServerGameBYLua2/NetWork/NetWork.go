...
func DealSendData(data string, msg string, mainCmd int, subCmd int) []byte {
	...
	// 生成数据包头部信息
	bufferHead := GetSendTcpHeaderData(uint16(mainCmd), uint16(subCmd), uint16(protoDataSize), uint16(msgSize))
	headSize := len(bufferHead)
	// 发送最后数据包
	bufferEnd := make([]byte, protoDataSize+headSize+msgSize)
...
func GetSendTcpHeaderData(maincmd uint16, childcmd uint16, size uint16, msgSize uint16) []byte {
	...
	binary.Write(bufferT,binary.LittleEndian,uint16(0))			// 服务器给客户端不用带token
