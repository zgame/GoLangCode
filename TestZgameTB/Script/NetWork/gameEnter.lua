...
--function HandleEnterScene(userId, buf)
----    print("------------客户端申请进入大厅-------------")
...
function SendEnterScene(userId, buf)
    local sendCmd = CMD_GameServer_pb.CMD_GF_EnterScene()
    sendCmd.game_type = GameTypeTB1000          -- 申请进入推币1000房间
    LuaNetWorkSendToUser(userId, MDM_GF_FRAME, SUB_GF_GAME_SCENE ,sendCmd,nil)
end

-- 服务器回复进入场景成功，同步玩家数据
function HandleEnterSceneUserData(userId, buf)
    local msg = CMD_Game_TB_pb.CMD_GF_S_ServerUserInfo()
    msg:ParseFromString(buf)

    --print(msg)
    --print("-------------------------------------------------------")
    --for coinId, value in ipairs(msg.user_lottery_list) do
    --    print("coinId"..coinId)
    --    print("lottery_type"..value.lottery_type)
    --    print("lottery_value"..value.lottery_value)
    --end
    --print("-------------------------------------------------------")
    --for coinId, value in ipairs(msg.user_big_coin_list) do
    --    print("coinId"..coinId)
    --    print("big_coin_value"..value.big_coin_value)
    --    print("big_coin_lottery_value"..value.big_coin_lottery_value)
    --    print("big_coin_type"..value.big_coin_type)
    --    print("draw_value"..value.draw_value)
    --end
