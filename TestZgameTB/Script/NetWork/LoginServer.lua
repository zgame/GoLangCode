...
-- 服务器返回登录服务器成功，下发uid
function HandleLoginGSGuestSuccess(serverId, buf)
    ...
    -- 下发uid之后， 开启一个定时器， 循环来call服务器
    SetNewTimer("NewTimerBy05Second",  uid , 1000)      -- lua 自己设定计时器 500毫秒
    --SetNewTimer("NewTimerBy4Second",  uid , 4000)      -- lua 自己设定计时器 4000毫秒
    SetNewTimer("NewTimerBy5Second",  uid , 5000)      -- lua 自己设定计时器 5000毫秒
...
-- 自己设定的新的计时器
function NewTimerBy05Second(userId)
    if GetPlayerByUID(userId) ~= nil then
        --print("这是lua自己设定的定时器"..uid)
        SendCastCoin(userId)
        SendObtainCoin(userId)
        --SendWheel(userId)
        --SendWheelDouble(userId)
        --SendWheelDouble(userId)
        --SendGameRoomInfo(userId)

        --SendGetMail(userId)
        --SendGetMailGift(userId)
        --SendDelMail(userId)
    end
end
function NewTimerBy5Second(userId)
    if GetPlayerByUID(userId) ~= nil then
        SendCard(userId)
        --SendTiger(userId)

        --SendBlood(userId)
        --SendBloodResult(userId)
        ...
        --SendClockScore(userId)
        --SendSignInScore(userId)
