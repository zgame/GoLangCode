...
function ByTable:SetFishData(fish,kindId, x, y, PathID)
    ...
    fish.FishType = GetExcelFishValue(kindId, "type")
    fish.CreateTime = now
    fish.TotalAliveTime = totalAliveTime
    fish.DeadTime = now + fish.TotalAliveTime * 1000
    fish.Speed = GetExcelFishValue(kindId, "speed")
    fish.PathID = PathID
    fish.Mulriple = GetExcelFishValue(kindId, "mulriple")
    fish.CapturePro = GetExcelFishValue(kindId, "capture_probability")
...
function ByTable:GetFishScore(fish)
    local score = GetExcelFishValue(fish.FishKindID,"gold_multiples")
    score = 100
    return score
end
