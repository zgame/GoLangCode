...
-- 接受网络消息
local function Login(serverId, buf)
    print("**************游戏服登录******************* ")
    local msg = Proto_Server.LogonUserID()
...
function GameNetwork.Receive(serverId, userId, mainSgId, subMsgId, data, token)
    ...
    elseif mainSgId == MDM_GR_USER then
    ...
    elseif mainSgId == MDM_GF_GAME then
        ...
        elseif subMsgId >= SUB_C_USE_SUMMON_GEM and subMsgId <= SUB_S_USER_DHS_INFO then
            --print("*************小海兽消息***************** ",userId)
            XhsReceiveMsg(serverId, userId, mainSgId, subMsgId, data, token)
        end
    elseif mainSgId == MAIN_CMD_ID then
        --print("*************日志服***************** ",serverId)
    ...
    elseif mainSgId == MDM_GR_HEARTBEAT then
    ...
    elseif mainSgId == MAIN_CHAT_SERVICE_INNER then
        -- 登录服 到 聊天服消息
    ...
    elseif mainSgId == MAIN_CHAT_SERVICE_CLIENT then
        -- 客户端 到 聊天服消息
