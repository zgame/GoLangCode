...
SandRockCreationItemExperimentNet ={}

-- 道具合成
function SandRockCreationItemExperimentNet.CreateItem(serverId, userId, buf)
    local msg = ProtoGameSandRock.CreationItemExperiment()
    msg:ParseFromString(buf)
    --print(msg)
    local createId = msg.createId
    local itemList = {}
    local player = GameServer.GetPlayerByUID(userId)
    local CreateItemId = CSV_creationItemExperiment.GetValue(createId, "OutItemId")
    if CreateItemId == nil then
        ZLog.Logger("实验台合成找不到CreateItemId" .. createId)
        return
    end
    ...
    --local Materials = CSV_creationItemExperiment.GetValue(createId, "Materials")
    ...
    --NetWork.Send(serverId, CMD_MAIN.MDM_GAME_SAND_ROCK, CMD_SAND_ROCK.SUB_CREATION_ITEM_EXPERIMENT, sendCmd, nil)
