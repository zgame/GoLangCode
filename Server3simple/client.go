...
// 获取消息, 在server handleClient 中被调用
func (c *Client) RecvMsg(serverClose <-chan bool) {
	// log
	fmt.Println("client recv msg start.id:%v,ip:%v", c.id, c.conn.RemoteAddr())
	defer fmt.Println("client recv msg quit.id:%v,ip:%v", c.id, c.conn.RemoteAddr())
	...
	for {				// 一直在循环的接收消息
		...
		fmt.Println("receive msg failed! id:%v,ip:%v,error:%v", c.id, c.conn.RemoteAddr(), e)

		return
	}
}

// 发送消息, 在server handleClient 中被调用
func (c *Client) SendMsg(serverClose <-chan bool) {
	// log
	fmt.Println("client send msg start.id:%v,ip:%v", c.id, c.conn.RemoteAddr())
	defer fmt.Println("client send msg quit.id:%v,ip:%v", c.id, c.conn.RemoteAddr())
	...
	for {				// 一直在循环的发送消息
		...
		fmt.Println("send msg failed! id:%v,ip:%v,error:%v", c.id, c.conn.RemoteAddr(), e)
