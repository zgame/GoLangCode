...
type clientMgr struct {
	...
	chAvail     chan int  // 可用客户端对象索引, 存放id的池子， 释放之后把空闲的id放进这里
...
func (mgr *clientMgr) gcClient() {
	// log
	fmt.Println("gc client start")
	defer fmt.Println("gc client end")
	...
	for id := range mgr.chGc {
		c := mgr.getClient(id)
		if nil == c {
			fmt.Println("client is nil! id=%v, max=%v", id, mgr.maxUsed)
			continue
		}
		...
		// log
		fmt.Println("client gc! free/peek:%v/%v", len(mgr.chAvail), mgr.maxUsed)
	}
}
...
func (mgr *clientMgr) createClient(conn net.Conn) (*Client, error) {
	...
	select {
	case clientId = <-mgr.chAvail:		// 如果有id池中已经释放掉的id， 那么赋值即可
	default:
		// 如果id池没有了， 那么就分配好了
	...
	if nil == client {
		fmt.Println("client is nil! id:", clientId)
		return nil, nil
	}
...
func (mgr *clientMgr) releaseClient(client *Client) {
	...
	// 回收
	mgr.chAvail <- id		//释放的id回归id池

	// log
	fmt.Println("client<%v> logoff", id)
}

// 获取客户端
func (mgr *clientMgr) getClient(id int) *Client {
	if !mgr.IsClientIdValid(id) {
		fmt.Println("invalid client id! id:%v", id)
		return nil
	}
