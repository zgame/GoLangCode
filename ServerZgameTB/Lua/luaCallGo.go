...
func (m *MyLua)InitResister() {
	...
	m.L.SetGlobal("luaCallGoNetWorkClose", m.L.NewFunction(luaCallGoNetWorkClose))		//注册到lua 网络关闭函数
...
func luaCallGoNetWorkSend(L *lua.LState) int {
	...
	//println("luaCallGoNetWorkSend          =-----------------",msg)
	...
	if userId == 0 {
		...
		// 给其他玩家发送消息
		//result =
			GetMyServerByUID(userId).SendMsg(data, msg, mainCmd, subCmd, token)	// 把客户端发来的token返回给客户端，标记出这是哪个消息的返回
