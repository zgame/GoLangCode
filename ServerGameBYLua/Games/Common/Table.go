...
type TableInterface interface{
	...
	PlayerSeat(seatId int, user UserInterface)	// 玩家坐到椅子上
	PlayerStandUp(seatId int, user UserInterface)		// 玩家离开椅子

	FireBullet(user UserInterface, lockFishId int)	int	// 发射一个新的子弹
	...
	GetUserInterfaceHandle() UserInterface		// 获取桌子管理玩家的句柄类型
	...
	SendSceneFishes(user UserInterface)				// 玩家登陆的时候，同步给鱼群数据
	...
	HitFish(userId UserInterface, bulletId int, fishId int)			// 击中一条鱼
...
type CommonTable struct {
	...
	UserSeatArray       map[int]UserInterface   // 座椅对应玩家uid的哈希表 ， key ： seat ，value： 玩家uid
	UserInterfaceHandle UserInterface // 玩家的句柄类型
	//UserCnt int			// 玩家数量
...
// 获取玩家的句柄类型
func (table *CommonTable) GetUserInterfaceHandle() UserInterface {
...
// ---------------------------玩家坐到椅子上-------------------------------
func (table * CommonTable)PlayerSeat(seatId int, user UserInterface)  {
...
func (table *CommonTable) InitTable()  {
	if table.CheckTableEmpty(){
		...
		table.UserSeatArray = make(map[int]UserInterface)
...
// --------------------------- 玩家离开椅子 -----------------------
func (table * CommonTable)PlayerStandUp(seatId int, user UserInterface) {
...
//----------------------------------------------------------------------------
// 玩家发射一个新的子弹
func (table *CommonTable)FireBullet(user UserInterface, lockFishId int) int{
...
//  击中一条鱼
func (table *CommonTable) HitFish(userid UserInterface, bulletid int, fishid int) {
	// 增加CD判断，不可以太频繁
...
func (table *CommonTable) SendSceneFishes(user UserInterface){
